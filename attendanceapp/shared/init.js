// shared/init.js — 兼容桥接层，统一模块导出的名字差异

window.AttendanceDB = new Proxy(DB, {
  get(target, prop) {
    if (prop === 'punches') return target.punch_records;
    if (prop === 'leaves') return target.leave_records;
    if (prop === 'attendance_auth') return target.attendance_auth;
    return target[prop];
  }
});
...
window.AttendanceRules = {
  ...
  async save(settings) {
    ...
    if (existing) {
      await DB.settings.put({key: 'attendance_config', value: settings});
    } else {
      await DB.settings.put({key: 'attendance_config', value: settings});
    }
  }
